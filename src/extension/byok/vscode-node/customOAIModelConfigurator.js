...
exports.CustomOAIModelConfigurator = void 0;
...
class CustomOAIModelConfigurator {
    constructor(_configurationService, _vendor, _provider) {
        this._configurationService = _configurationService;
        this._vendor = _vendor;
        this._provider = _provider;
        this._configKey = configurationService_1.ConfigKey.CustomOAIModels;
        this._forceRequiresAPIKey = false;
        if (_vendor === 'azure') {
            this._forceRequiresAPIKey = true;
            this._configKey = configurationService_1.ConfigKey.AzureModels;
        }
    }
    async configureModelOrUpdateAPIKey() {
        const options = [
            {
     ...
     */
    async configure(isProvideLMInfoCall) {
        while (true) {
            ...
            quickPick.ignoreFocusOut = true;
            quickPick.buttons = items.length > 1 ? [vscode_1.QuickInputButtons.Back] : [];
            const selected = await new Promise((resolve) => {
     ...
     */
    async _editModel(modelId, currentConfig) {
        const items = [
            {
        ...
        if (selected.label.includes('Delete')) {
            const confirmed = await vscode_1.window.showWarningMessage(`Are you sure you want to delete the model "${currentConfig.name}"?`, { modal: true }, 'Delete');
            if (confirmed === 'Delete') {
     ...
     */
    async _editModelConfig(currentConfig) {
        // Edit Name
        const modelName = await this._createInputBoxWithBackButton({
     ...
     */
    async _selectCapabilities(defaults) {
        const capabilities = {
     ...
     */
    async _configureTokenLimits(defaults) {
        // Input tokens
     ...
     */
    _createInputBoxWithBackButton(options) {
        const disposableStore = new lifecycle_1.DisposableStore();
...
exports.CustomOAIModelConfigurator = CustomOAIModelConfigurator;
//# sourceMappingURL=customOAIModelConfigurator.js.map