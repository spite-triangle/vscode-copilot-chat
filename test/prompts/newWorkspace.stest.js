...
const generateNewWorkspaceContent_1 = require("../../src/extension/intents/node/generateNewWorkspaceContent");
...
function generateFileContentsPromptTest(scenario, language) {
    (0, stest_1.stest)({ description: `File contents generation: ${scenario.args.FILEPATH} in ${scenario.args.PROJECT_DESCRIPTION}`, language }, async (testingServiceCollection) => {
        const accessor = testingServiceCollection.createTestingAccessor();
        const fileContentGenerator = accessor.get(instantiation_1.IInstantiationService).createInstance(generateNewWorkspaceContent_1.FileContentsGenerator);
        const promptArgs = {
        ...
        const result = await fileContentGenerator.generate(promptArgs, cancellation_1.CancellationToken.None);
        const trimmedResult = result.trim();
        // Generated file contents should never be empty
        assert_1.default.ok(trimmedResult.length > 0);
        // Generated file contents should never start with backticks
        assert_1.default.ok(!trimmedResult.startsWith('```'));
        // The AI shouldn't refuse to generate file contents
        assert_1.default.ok(!trimmedResult.startsWith('Sorry, I cannot generate file contents'));
...
var TypeScript;
(function (TypeScript) {
    TypeScript.PROJECT_DESCRIPTION = 'Create a TypeScript Express app';
    TypeScript.PROJECT_TREE_STRUCTURE = `
	my-express-app
	├── src
	│   ├── app.ts
	│   ├── controllers
	...
	├── tsconfig.json
	└── README.md`;
    TypeScript.PROJECT_SPECIFICATION = `The project has the following files:
...
(function (PythonDjango) {
    PythonDjango.PROJECT_DESCRIPTION = 'python Django backend that uses REST API to connect to Azure CosmosDB NoSQL';
    PythonDjango.PROJECT_TREE_STRUCTURE = `
	...
	├── .gitignore
	├── README.md`;
    PythonDjango.PROJECT_SPECIFICATION = `The project tree structure consists of the following files and directories:
...
})(PythonDjango || (PythonDjango = {}));
var CPP;
(function (CPP) {
    CPP.PROJECT_DESCRIPTION = 'A nodejs native node module that has a fib function in C++ exported to TS using NAPI. Include a tsconfig.json. The fib function takes an integer N as its argument.';
    CPP.PROJECT_TREE_STRUCTURE = `
my-node-module
├── src
│   ├── fibModule.cpp
│   ├── fibModule.h
...
(0, stest_1.ssuite)({ title: 'new', subtitle: 'prompt', location: 'panel' }, () => {
    // #region TypeScript
    generateFileContentsPromptTest({
        ...
        validate: (response) => {
            assert_1.default.ok(response.startsWith('#'), `Generated README.md does not start with a #`);
        }
    }, 'typescript');
    generateFileContentsPromptTest({
        args: {
        ...
        validate: (response) => {
            try {
                ...
                assert_1.default.fail(`Generated package.json is not valid JSON: ${JSON.stringify(ex)}`);
            }
        }
    }, 'typescript');
    generateFileContentsPromptTest({
    ...
    generateFileContentsPromptTest({
        ...
        validate: (response) => {
            assert_1.default.ok(response.includes('# This file is intentionally left blank.'), 'Intentionally blank file does not contain the expected comment');
        }
    }, 'python');
    ...
    generateFileContentsPromptTest({
        ...
        validate: (response) => {
            assert_1.default.ok(!response.startsWith('++'), 'C++ file was not correctly parsed');
        }
    }, 'cpp');
    generateFileContentsPromptTest({
        args: {
