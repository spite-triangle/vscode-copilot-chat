...
const settingsEditorSearchResultsSelector_1 = require("../../src/extension/prompt/node/settingsEditorSearchResultsSelector");
...
(0, stest_1.ssuite)({ title: 'settingsEditorSearchResultsSelector', location: 'external' }, () => {
    stest_1.stest.skip({ description: 'Selects expected command center setting' }, async (testingServiceCollection) => {
        const accessor = testingServiceCollection.createTestingAccessor();
        const instantiationService = accessor.get(instantiation_1.IInstantiationService);
        const selector = instantiationService.createInstance(settingsEditorSearchResultsSelector_1.SettingsEditorSearchResultsSelector);
        const settingsList = [
            {
        ...
        const results = await selector.selectTopSearchResults(endpoint, 'Hide search bar at top of window', settingsList, cancellation_1.CancellationToken.None);
        assert_1.default.ok(results.length > 0, 'No settings were selected');
        assert_1.default.ok(results.some(result => result === 'window.commandCenter'), 'Expected setting "window.commandCenter" was not found');
    });
    (0, stest_1.stest)({ description: 'Selects expected agent mode setting' }, async (testingServiceCollection) => {
        const accessor = testingServiceCollection.createTestingAccessor();
        const instantiationService = accessor.get(instantiation_1.IInstantiationService);
        const selector = instantiationService.createInstance(settingsEditorSearchResultsSelector_1.SettingsEditorSearchResultsSelector);
        const settingsList = [
        ...
        const endpointProvider = accessor.get(endpointProvider_1.IEndpointProvider);
        const endpoint = await endpointProvider.getChatEndpoint('copilot-base');
        const results = await selector.selectTopSearchResults(endpoint, 'agentmode', settingsList, cancellation_1.CancellationToken.None);
        assert_1.default.ok(results.length > 0, 'No settings were selected');
        assert_1.default.ok(results.some(result => result === 'chat.agent.enabled'), 'Expected setting "chat.agent.enabled" was not found');
    });
});
//# sourceMappingURL=settingsEditorSearchResultsSelector.stest.js.map